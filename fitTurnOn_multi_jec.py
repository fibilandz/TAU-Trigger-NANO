...
def MinTarget(dy, eff):
    y = np.cumsum(dy)
    # return np.sum((eff.y - y) ** 2 / np.fmax(1e-6, np.abs(eff.y_error_high + eff.y_error_low)))
    # return np.sum(((eff.y - y) / (eff.y_error_high + eff.y_error_low)) ** 2)
    return np.sum((eff.y-y)**2)
...
class FitResults:
    def __init__(self, eff, x_pred):
        ...
        res = scipy.optimize.minimize(MinTarget, np.zeros(N), args=(eff,), bounds = [ [0, 1] ] * N,
                                      options={"maxfun": int(1e6), "maxiter": int(1e6)})
        if not res.success:
            print(res)
        ...
        self.gp_high = GaussianProcessRegressor(kernel=kernel_high, alpha=yerr[high_pt] ** 2, n_restarts_optimizer=50)
...
ch_validity_thrs = { 'ditaujet_jetleg60': 60 }
...
abs_jet_eta_cuts = ['|jet #eta| < 2.5','2.5 < |jet #eta| < 3','3 < |jet #eta| < 5']
for channel in channels:
    with PdfPages('{}_{}.pdf'.format(args.output, channel)) as pdf:
        ...
        # for var_mc in ["_"]:
            for eta_cut in abs_jet_eta_cuts:
                eta_cut_label = eta_cut.strip().replace("<", "_").replace(">", "_")
                name_pattern = '{}_{}_{{}}_{}_fit_eff'.format(var_mc, eta_cut_label, channel)
                ...
                print(var_mc)
                if len(var_mc) > 2:
                    title = "Turn-ons for {} trigger, eta {}, with {} syst".format(channel, eta_cut, var_mc[1:])
                else:
                    title = "Turn-ons for {} trigger, eta {}".format(channel, eta_cut)
                # if dm != 'all':
                ...
                out_name_pattern = '{{}}_{}_{}_{}_{{}}'.format(var_mc, channel, eta_cut_label)
