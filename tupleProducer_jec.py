...
class tupleProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        if self.isMC:
            self.out.branch("jet_pt_corr", "F")
            self.out.branch("jet_pt_corr_Regrouped_BBEC1_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_BBEC1_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_HF_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_HF_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_EC2_2024_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_EC2_2024_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_EC2_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_EC2_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_RelativeSample_2024_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_RelativeSample_2024_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_BBEC1_2024_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_BBEC1_2024_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_Absolute_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_Absolute_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_FlavorQCD_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_FlavorQCD_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_HF_2024_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_HF_2024_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_RelativeBal_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_RelativeBal_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_Total_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_Total_up", "F")
            self.out.branch("jet_pt_corr_Regrouped_Absolute_2024_down", "F")
            self.out.branch("jet_pt_corr_Regrouped_Absolute_2024_up", "F")
            self.out.branch("jet_pt_corr_Total_down", "F")
            self.out.branch("jet_pt_corr_Total_up", "F")
        ...
        self.out.branch("jet_trig_obj_idx", "I")
    ...
    def collectJets(self, signalMu_v4, signalTau_v4, jets, deltaR2Thr):
        gen = 1
        pt = 2
        mva = 4
        deepTau = 8
        best_jet = []
        for _j, _jet in enumerate(jets):
            _jet_v4 = _jet.p4()
            if (_jet_v4.Pt() > 18) and (abs(_jet_v4.Eta()) < 4.7) and _jet.jetId > 2 and (self.deltaR2(signalMu_v4, _jet_v4) > deltaR2Thr) and (self.deltaR2(signalTau_v4, _jet_v4) > deltaR2Thr):
                best_jet.append( (_jet_v4, _jet, _j) )
        if (self.nanoVer == 10 or self.nanoVer == 11):
            best_jet.sort(key=lambda x:(x[0].Pt()) ,reverse=True)
        else:
            best_jet.sort(key=lambda x:(x[0].Pt()) ,reverse=True)
    ...
    def collectTaus(self, signalMu_v4, taus, genleptons, deltaR2Thr):
        ...
        if genleptons is None:
            ...
            selected_gen_tau = self.selectGenLeg(genleptons)
        #
    ...
    def selectGenLeg(self, genleptons):
    ...
    def triggerMatch(self, ref_p4, trig_objs, isMuon, isTau): # isMuon : True for muon, False for tau
    ...
    def analyze(self, event):
        ...
        muon_trig_obj_idx =  self.triggerMatch(signalMu_v4, trigobjs, True, False)

        # add direct HLT_IsoMu27 judgement
        if HLT.IsoMu24 > 0.5 and muon_trig_obj_idx is not None:
            tag_trig_match = True
        ...
        tau_trig_obj_idx =  self.triggerMatch(tau_ref_p4, trigobjs, False, True)
        ...
        jets = self.collectJets(signalMu_v4, tau_ref_p4, jets, deltaR2Thr)
        jet_trig_obj_idx = -999
        if len(jets) > 0:
            jet_v4 = jets[0][0]     # muon_ref_p4
            jet = jets[0][1]
            jet_trig_obj_idx = self.triggerMatch(jet_v4, trigobjs, False, False)
        ...
        if (gen_muon is not None):
            ...
            self.out.fillBranch("muon_gen_charge", int((-gen_muon.pdgId) / abs(gen_muon.pdgId)) )
        else:
        ...
        if has_gen_tau:
            ...
            self.out.fillBranch("tau_gen_charge", int((-gen_tau.pdgId) / abs(gen_tau.pdgId)))
        ...
        if len(jets) > 0:
            self.out.fillBranch("jet_eta", jet_v4.Eta())
            self.out.fillBranch("jet_phi", jet_v4.Phi())
            if self.isMC:
                # self.out.fillBranch("jet_pt", jet.pt_nom)
                self.out.fillBranch("jet_pt_corr", jet.pt_corr)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_down", jet.pt_corrRegrouped_BBEC1_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_up", jet.pt_corrRegrouped_BBEC1_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_down", jet.pt_corrRegrouped_HF_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_up", jet.pt_corrRegrouped_HF_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_2024_down", jet.pt_corrRegrouped_EC2_2024_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_2024_up", jet.pt_corrRegrouped_EC2_2024_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_down", jet.pt_corrRegrouped_EC2_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_up", jet.pt_corrRegrouped_EC2_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeSample_2024_down", jet.pt_corrRegrouped_RelativeSample_2024_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeSample_2024_up", jet.pt_corrRegrouped_RelativeSample_2024_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_2024_down", jet.pt_corrRegrouped_BBEC1_2024_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_2024_up", jet.pt_corrRegrouped_BBEC1_2024_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_down", jet.pt_corrRegrouped_Absolute_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_up", jet.pt_corrRegrouped_Absolute_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_FlavorQCD_down", jet.pt_corrRegrouped_FlavorQCD_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_FlavorQCD_up", jet.pt_corrRegrouped_FlavorQCD_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_2024_down", jet.pt_corrRegrouped_HF_2024_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_2024_up", jet.pt_corrRegrouped_HF_2024_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeBal_down", jet.pt_corrRegrouped_RelativeBal_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeBal_up", jet.pt_corrRegrouped_RelativeBal_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_Total_down", jet.pt_corrRegrouped_Total_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_Total_up", jet.pt_corrRegrouped_Total_up)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_2024_down", jet.pt_corrRegrouped_Absolute_2024_down)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_2024_up", jet.pt_corrRegrouped_Absolute_2024_up)
            else:
                self.out.fillBranch("jet_pt", jet.pt)
        else:
            self.out.fillBranch("jet_pt", -999.)
            self.out.fillBranch("jet_eta", -999.)
            self.out.fillBranch("jet_phi", -999.)
            if self.isMC:
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_2024_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_2024_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_EC2_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeSample_2024_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeSample_2024_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_2024_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_BBEC1_2024_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_FlavorQCD_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_FlavorQCD_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_2024_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_HF_2024_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeBal_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_RelativeBal_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Total_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Total_up", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_2024_down", -999.)
                self.out.fillBranch("jet_pt_corr_Regrouped_Absolute_2024_up", -999.)
                self.out.fillBranch("jet_pt_corr_Total_down", -999.)
                self.out.fillBranch("jet_pt_corr_Total_up", -999.)
        ...
        if jet_trig_obj_idx is not None:
            self.out.fillBranch("jet_trig_obj_idx", jet_trig_obj_idx)
        else:
            self.out.fillBranch("jet_trig_obj_idx", -999)
